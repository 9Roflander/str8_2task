(() => {
  ...
  var BACKEND_WS_URL = "ws://localhost:5167/ws/extension";
    ...
    async handlePostMessage(message) {
      ...
      try {
        ...
        const meetingTab = tabs.find(
          (tab) => tab.url && (tab.url.includes("meet.google.com") || tab.url.includes("zoom.us") || tab.url.includes("teams.microsoft.com") || tab.url.includes("teams.live.com"))
        );
        if (!meetingTab) {
  ...
  var wsManager = new WebSocketManager(BACKEND_WS_URL);
