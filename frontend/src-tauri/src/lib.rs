...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|_app| {
            ...
            // Initialize Core Audio backend for system audio capture on macOS
            // This ensures the app uses Core Audio tap (which can capture all app audio)
            // instead of ScreenCaptureKit (which may have permission issues)
            #[cfg(target_os = "macos")]
            {
                use audio::capture::{AudioCaptureBackend, set_current_backend};
                log::info!("🔊 Setting audio capture backend to Core Audio for app audio capture");
                set_current_backend(AudioCaptureBackend::CoreAudio);
                log::info!("✅ Core Audio backend configured - will capture audio from all apps");
            }

            // Trigger system audio permission request on startup
            // This will prompt the user for Audio Capture permission (macOS 14.4+)
            #[cfg(target_os = "macos")]
            {
                log::info!("🔐 Triggering Audio Capture permission check...");
                tauri::async_runtime::spawn(async {
                    // Small delay to ensure app window is visible before permission dialog
                    tokio::time::sleep(std::time::Duration::from_secs(2)).await;
                    if let Err(e) = audio::permissions::trigger_system_audio_permission() {
                        log::warn!("⚠️ Failed to trigger system audio permission: {}", e);
                        log::info!("💡 To capture meeting audio, grant Audio Capture permission in System Settings → Privacy & Security → Audio Capture");
                    }
                });
            }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            audio::recording_commands::toggle_microphone_mute,
            audio::recording_commands::is_microphone_muted,
            ...
            api::api_send_questions_to_chat,
            api::api_generate_clarifying_questions,
            api::api_ping_extensions,
            // Summary commands
