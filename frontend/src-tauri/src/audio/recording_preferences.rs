...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\str8_2task-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("str8_2task-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("str8_2task-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/str8_2task-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("str8_2task-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/str8_2task-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("str8_2task-recordings")
    }
